...
export const generateCrazyThursdayCopy = async (params: GenerateParams): Promise<CopyResult> => {
  ...
  const backgroundPlot = `
    ...
    \t•\t江叔江晏：主角的义父，年轻时曾在渡桥之战中保护幼子逃难。其随身携带的武林至宝“镇冠珏”被契丹与绣金楼追捕。十三年后归来，为了保护主角吸引敌方火力而夺走主角的镇冠珏。
  ...
  const prompt = `
    ...
    1. 沉浸式乱世剧情
      - 氛围必须逼真：残阳、断刃、废墟、骤雨、荒道上的血腥味、压得人喘不过气的命运感。
      - 绝对禁止出现任何“功能性游戏词汇”（如：点穴、技能、机制、开放世界等）。
      - 文风冷、狠、短，像刀锋刮过耳边。
    2. 刀与宿命
      - “刀”的象征要贯穿全文：杀伐、疲惫、犹疑、背负。
      - 让玩家能读出暗示 —— 那种明明一句没提游戏，却能懂其中江湖隐痛的感觉。
    3. 反转结构
     - 前 90% 必须极度压抑、肃杀、像战场后的独行者喃喃。
     - 最后突然转折，V我50，肯德基疯狂星期四。复仇场景加一句，且听我如何复仇。
     - 反转要冷，要硬，要荒诞得合理。
    4. **字数限制**：全文字数严格控制在50-200字之间。
    5. **关键词融入**：${keywords || '完全随机'}。
